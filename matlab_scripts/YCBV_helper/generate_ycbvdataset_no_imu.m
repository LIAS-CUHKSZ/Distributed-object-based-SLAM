...
datapath=['E:\datasets\YCB-V\data2\data\', seq, '\'];

savepath=['E:\datasets\YCB-V\data2\data\',seq,'split\'];
...
constant_v_list=zeros(agent_num,3);

for i=1:agent_num


    constant_vi=zeros(6,3);
    for z=2:7
        dT=  invT(Robot(i).real_path(z-1).T) * Robot(i).real_path(z).T;
        pi = dT(1:3,4)';
        

        constant_vi(z-1,:) = pi;
    end
    constant_v_list(i,:) = mean(constant_vi)/dti;
